...
#' 
#' 
#' Hybrid estimation for given SEM
#' 
...
#' @param covarRestrict
#' @export
hybridSEM <- function(Y, B, Omega, B.hat = NULL, Omega.hat = NULL, mu.hat = NULL, meanEst = T,
                  covarRestrict = NULL, tol = 1e-4, maxIter = 500){
    ...
    if(meanEst == F & sum(B) == 0){
      ...
      if(is.null(B.hat) ) {
        out_ricf_init <- ricf(B = B, Omega = Omega, Y = Y - rowMeans(Y), BInit = NULL,
                              OmegaInit = NULL, sigConv = 0, maxIter = maxIter,
      ...
      if(meanEst){
        ...
        Omega.hat <- round((temp %*% diag(p) %*% t(temp)) * Omega,8)
      } else {
        B.hat <- matrix(0, nrow = V, ncol = V)
