sempl_mg_search1 <- function(Y, moments = 3,
                            null_cutoff = qchisq(.9, df = 2 + 1 + (moments - 2) * 2),
                            null_cutoff1 = 10,
  ...
  while(change_made & (dim(E_remaining)[1] > 0)){
    ...
    for(i in 1:dim(E_remaining)[1]){
      ...
      if( out.00$lr < null_cutoff ) {
        ...
        if(out.21$lr - out.12$lr> null_cutoff1) {
        ...
        } else if(out.12$lr - out.21$lr  > null_cutoff1) {
