...
#' @export
fbmgs_mod <- function(Y, moments = 4, meanEst = F, outerTol = 1e-3, innerTol = 1e-3,
  ...
  for(i in 2:V){
    for(j in 1:(i-1)) {
      ...
      if (el_fit_null > fwdNullTol) {
        ...
        if ( abs(el_fit_1 - el_fit_2) <  fwdDirectTol | min(el_fit_1, el_fit_2) > fwdNullTol )  {
          ## If neither directed edge works well, or both work reasonably well,
          ## Then we add a bidirected edge
  ...
  for (z in 1:dim(bidirected_prune)[1]) {
    ...
    B_i_to_j <- B_j_to_i <- B <-  B
    B_i_to_j[j, i] <- 1
    B_j_to_i[i, j] <- 1
    
    el_fit_null <- sempl_cd(Y = Y, B = B, Omega = Omega.test, meanEst = meanEst, 
    ...
    if( max(el_fit_null, el_fit_1, el_fit_2) - el_fit_saturated < bwdNullTol) {
      Omega <- Omega.test
      if(printStatus){
        cat("Bidirected Edge removed; ")
        cat(paste("null: ", round(el_fit_null,1), "i -> j: ", round(el_fit_1, 1), " j -> i: ", round(el_fit_2, 1),
                    " Sat: ", round(el_fit_saturated, 1), sep = ""))
      }
      
      el_fit_saturated <- el_fit_null
    } else {
    if( (el_fit_1 - el_fit_saturated) < bwdNullTol & el_fit_2 - el_fit_1 > bwdDirectTol ){
      B <- B_i_to_j
      Omega <- Omega.test
      directed_prune <- rbind(directed_prune, c(j, i, abs(el_fit_2 - el_fit_2), 0))
    ...
    } else if ((el_fit_2 - el_fit_saturated) < bwdNullTol & el_fit_1 - el_fit_2 > bwdDirectTol) {
      B <- B_j_to_i
      Omega <- Omega.test
      directed_prune <- rbind(directed_prune, c(i, j, el_fit_1 - el_fit_2, 0))
  ...
  for (z in 1:dim(directed_prune)[1]) {
    ...
    B_null <- B
    B_null[i, j] <- 0
    el_fit_null <- sempl_cd(Y = Y, B = B_null, Omega = Omega, meanEst = meanEst, 
                            covarRestrict = .getCovarRestrict(Omega, moments = moments), tol = innerTol,
                            outerTol = outerTol)
    ...
    if(null.diff < bwdNullTol) {
      ...
      if(printStatus){
        ...
        cat(paste("Null: ", round(el_fit_null,1), "; Saturated: ", round(el_fit_saturated,1), sep = ""))
      }
    } else {
      if(printStatus){
